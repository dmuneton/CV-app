...
import { OrderItem, BOMComponent, InventoryItem, OrderProductLine } from '../../types';
...
// One editable "producto" block — for a normal single-product order there's always
// exactly one of these (id 'legacy'), wrapping the order's top-level bomComponents.
// A multi-product order (Órdenes → "Añadir Producto") gets one per entry in
// order.products, each edited independently.
...
export const EditOrderModal: React.FC<EditOrderModalProps> = ({
  ...
  const isMultiProduct = editableProducts.length > 1;
      ...
      if (order.products && order.products.length > 0) {
        setEditableProducts(
          order.products.map((p) => ({
            ...
            bomComponents: JSON.parse(JSON.stringify(p.bomComponents || []))
          }))
        );
      } else {
        setEditableProducts([
          {
            id: 'legacy',
            productName: order.productSpec,
            itemsCount: order.itemsCount || 1,
            bomComponents: order.bomComponents ? JSON.parse(JSON.stringify(order.bomComponents)) : []
          }
        ]);
      }
    }
  ...
  const updateProduct = (id: string, updater: (p: EditableProduct) => EditableProduct) => {
    setEditableProducts((prev) => prev.map((p) => (p.id === id ? updater(p) : p)));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const isProductsOrder = !!order.products && order.products.length > 0;

    const updatedProducts: OrderProductLine[] | undefined = isProductsOrder
      ? editableProducts.map((ep) => {
          const original = order.products!.find((p) => p.id === ep.id);
          return {
            id: ep.id,
            productName: ep.productName,
            itemsCount: ep.itemsCount,
            salePrice: original?.salePrice ?? 0,
            unitCost: ep.bomComponents.reduce((acc, i) => acc + i.totalCost, 0),
            bomComponents: ep.bomComponents
          };
        })
      : undefined;
    ...
    onSave({
      ...
      value: Number(value),
      // Para varias órdenes-producto la cantidad real vive por producto — el total
      // se recalcula solo. Para una sola, sigue siendo el campo editable de arriba.
      itemsCount: isMultiProduct
        ? editableProducts.reduce((acc, p) => acc + p.itemsCount, 0)
        : Number(itemsCount),
      ...
      // Órdenes de un solo producto sin products: se sigue guardando en el campo
      // clásico de siempre.
      bomComponents: isProductsOrder ? order.bomComponents : editableProducts[0]?.bomComponents
    });
    onClose();
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              {isMultiProduct ? (
                <>
                  <div className="w-full bg-[#eef5f7] border border-[#c1c8c2] rounded-lg px-3.5 py-2 text-sm font-numeric-data text-[#414844]">
                    {editableProducts.reduce((acc, p) => acc + p.itemsCount, 0)}
                  </div>
                  <p className="text-[10px] text-[#717973] mt-1">
                    Suma automática — ajusta las unidades de cada producto más abajo.
                  </p>
                </>
              ) : (
                <input
          ...
          {/* Insumos / BOM Editing — un bloque por producto. La mayoría de órdenes
              tienen un solo producto (bloque sin título); las de varios productos
              ("Añadir Producto" en Órdenes) muestran uno por cada uno, cada cual con
              sus propias unidades e insumos. */}
          <div className="space-y-4">
            {editableProducts.map((p) => (
              <InsumosEditorBlock
                key={p.id}
                title={isMultiProduct ? p.productName : undefined}
                itemsCount={p.itemsCount}
                onItemsCountChange={
                  isMultiProduct ? (n) => updateProduct(p.id, (prev) => ({ ...prev, itemsCount: n })) : undefined
                }
                bomComponents={p.bomComponents}
                onUpdateBOM={(updated) => updateProduct(p.id, (prev) => ({ ...prev, bomComponents: updated }))}
                inventory={inventory}
                datalistId={`edit-order-inventory-datalist-${p.id}`}
              />
            ))}
          </div>
          ...
          {confirmingDelete ? (
            <div className="pt-4 border-t border-[#c1c8c2] space-y-3">
              <div className="bg-[#ffdad6]/40 border border-[#ba1a1a]/30 rounded-lg p-3 space-y-1.5">
                <p className="text-xs font-semibold text-[#93000a] flex items-center gap-1.5">
                  <span className="material-symbols-outlined text-[16px]">warning</span>
                  <span>¿Borrar definitivamente la orden {order.orderId}?</span>
                </p>
...
// ---------------------------------------------------------------------------------
// InsumosEditorBlock: la lista de insumos editable de un producto — añadir/quitar
// insumos, ajustar cantidades y (para mano de obra) el costo unitario. Se usa una
// vez por producto: sin título para una orden normal de un solo producto, y una vez
// por cada uno cuando la orden tiene varios ("Añadir Producto" en Órdenes).
// ---------------------------------------------------------------------------------

interface InsumosEditorBlockProps {
  title?: string;
  itemsCount: number;
...
const InsumosEditorBlock: React.FC<InsumosEditorBlockProps> = ({
  title,
  itemsCount,
  onItemsCountChange,
  bomComponents,
  ...
  // Not a <form onSubmit> handler on purpose: this modal already lives inside the
  // outer <form>, and a nested <form> here would bubble its submit up and close the
  // whole modal prematurely (nested forms are invalid HTML anyway).
  const handleAddComponent = () => {
    ...
    onUpdateBOM([...bomComponents, newComponent]);
    setNewCompName('');
    setNewCompQty(1);
    setNewCompCost(500);
  ...
  const handleRemoveComponent = (id: string) => {
    onUpdateBOM(bomComponents.filter((item) => item.id !== id));
  };
  ...
  return (
    <div className={title ? 'bg-[#f4fafd] border border-[#c1c8c2] rounded-xl p-3' : ''}>
      {title && (
        <div className="flex items-center justify-between gap-3 mb-2.5">
          <span className="text-xs font-bold text-[#012d1d]">{title}</span>
          {onItemsCountChange && (
            <div className="flex items-center gap-1.5">
              <label className="font-label-caps text-[9px] text-[#414844]">Unidades:</label>
              <input
                type="number"
                min="1"
                value={itemsCount}
                onChange={(e) => onItemsCountChange(Math.max(1, Number(e.target.value)))}
                className="w-16 bg-white border border-[#c1c8c2] rounded-lg px-2 py-1 text-xs font-numeric-data text-[#161d1f] focus:outline-none focus:border-[#0284c7]"
              />
            </div>
          )}
        </div>
      ...
      {/* Quick Add Insumo Inline Form — a plain div, not a <form>: see note above. */}
      {isAddingComponent && (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[440px]">
              ...
              <tbody className="text-xs divide-y divide-[#c1c8c2]/50">
                {bomComponents.map((item) => (
                  <tr key={item.id} className={item.isLabor ? 'bg-[#e8eff1]/60 font-medium' : ''}>
